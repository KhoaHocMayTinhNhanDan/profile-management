...
from .generate_document_from_template_dto import (
    GenerateDocumentFromTemplateInput,
    GenerateDocumentFromTemplateOutput,
)
import uuid
import os
...
class GenerateDocumentFromTemplateInteractor:
    ...
    async def execute(
        self, input_data: GenerateDocumentFromTemplateInput
    ) -> GenerateDocumentFromTemplateOutput:
        ...
        try:
            ...
            return GenerateDocumentFromTemplateOutput(
                status="error", message=f"Failed to generate docx: {e}"
            )
        ...
        # Check if a document with the same output name already exists in the profile
        existing_doc = None
        for doc in profile.documents:
            if doc.name == input_data.output_doc_name:
                existing_doc = doc
                break

        if existing_doc:
            # Overwrite properties of the existing document
            existing_doc.url = generated_url
