...
class CreateProfileInteractor:
    ...
    async def execute(self, input_data: CreateProfileInput) -> CreateProfileOutput:
        if not input_data.profile_id:
            return CreateProfileOutput(status="error", message="Profile ID cannot be empty.")

        existing_profile = await self._repository.get_profile(input_data.profile_id)
        if existing_profile:
            return CreateProfileOutput(status="error", message=f"Profile with ID '{input_data.profile_id}' already exists.")

        template = await self._repository.get_template(input_data.template_id)
        if not template:
            return CreateProfileOutput(status="error", message=f"Profile template '{input_data.template_id}' not found.")
        ...
        if not is_valid:
            return CreateProfileOutput(status="error", message="Validation failed.", errors=validation_errors)
        ...
        return CreateProfileOutput(status="success", message=f"Profile '{input_data.profile_id}' created successfully as Draft.")
