...
router = APIRouter(prefix="/api/v1/generate-document-from-template", tags=["GenerateDocumentFromTemplate"])
...
@router.post("/")
async def generate_document_from_template_endpoint(request: GenerateDocumentFromTemplateRequest, context: AppContextWeb = Depends(get_context)):
    ...
    try:
        response = await controller.execute(GenerateDocumentFromTemplateInput(
