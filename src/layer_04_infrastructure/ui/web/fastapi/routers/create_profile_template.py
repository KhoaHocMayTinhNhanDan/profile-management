...
from src.layer_02_usecases.usecases.create_profile_template.create_profile_template_dto import (
...
router = APIRouter(
    prefix="/api/v1/create-profile-template", tags=["CreateProfileTemplate"]
)
...
@router.post("/")
async def create_profile_template_endpoint(
    request: CreateProfileTemplateRequest, context: AppContextWeb = Depends(get_context)
):
