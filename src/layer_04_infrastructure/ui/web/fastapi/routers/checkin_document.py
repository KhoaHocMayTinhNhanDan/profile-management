...
@router.post("/")
async def checkin_document_endpoint(request: CheckinDocumentRequest, context: AppContextWeb = Depends(get_context)):
    ...
    try:
        response = await controller.execute(CheckinDocumentInput(
