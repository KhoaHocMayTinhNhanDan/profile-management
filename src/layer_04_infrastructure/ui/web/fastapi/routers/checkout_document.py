...
@router.post("/")
async def checkout_document_endpoint(request: CheckoutDocumentRequest, context: AppContextWeb = Depends(get_context)):
    ...
    try:
        response = await controller.execute(CheckoutDocumentInput(
