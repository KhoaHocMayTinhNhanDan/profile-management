...
@router.post("/")
async def update_profile_endpoint(request: UpdateProfileRequest, context: AppContextWeb = Depends(get_context)):
    ...
    try:
        response = await controller.execute(UpdateProfileInput(
