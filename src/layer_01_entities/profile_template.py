...
class ProfileTemplate:
    def __init__(self, template_id: str, name: str, fields_schema: list):
    ...
    def validate_data(self, data: dict) -> tuple[bool, list[str]]:
        ...
        for field_name, field_def in schema_dict.items():
            if field_def.get("required", False) and field_name not in data:
                errors.append(f"Field '{field_name}' ({field_def.get('label', field_name)}) is required.")
        
        # Validate field types
        for key, value in data.items():
            if key not in schema_dict:
                # Allow extra fields or raise error? Usually DMS allows only defined fields.
                errors.append(f"Field '{key}' is not defined in template '{self.name}'.")
                continue
            ...
            if expected_type == "number":
                ...
                if isinstance(value, str):
                    if not re.match(r"^\d{4}-\d{2}-\d{2}$", value):
                        errors.append(f"Field '{key}' must be a date in YYYY-MM-DD format.")
                    else:
                        try:
