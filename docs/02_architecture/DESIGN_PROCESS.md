...
- [ ] **Bước B (Auto-Generate):** 
    - Chạy Project Manager App (CLI hoặc GUI):
      - CLI: `python scripts/run/cli/run_project_manager.py`
      - GUI: `python scripts/run/desktop/run_project_manager.py`
...
- [ ] **Bước D (Kiểm chứng):** Chạy Unit Test (`pytest tests/unit/`) hoặc test bằng Mock DB trước khi ráp Data thật.
