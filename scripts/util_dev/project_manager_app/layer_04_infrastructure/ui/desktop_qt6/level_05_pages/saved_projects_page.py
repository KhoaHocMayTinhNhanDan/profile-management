import os
from PyQt6.QtWidgets import QVBoxLayout, QHBoxLayout, QFrame, QListWidget, QListWidgetItem
...
class SavedProjectsPage(BasePageTemplate):
    ...
    def __init__(self, main_win, app_ctx, root_dir):
        ...
        self.save_desc = BodyLabel("Saves src/ and tests/ folders into a named backup archive.")
        ...
        proj = self.main_win.get_project_name() if hasattr(self.main_win, 'get_project_name') else ""
        if proj:
    ...
    def refresh_project_list(self):
        ...
        try:
            ...
            if not projs:
                placeholder = QListWidgetItem(self.i18n_manager.translate("(No saved projects found)"))
    ...
    def handle_load_project(self):
        ...
        reply = NotificationDialog.ask_question(self, 'Confirm Restore', 
    ...
    def retranslate_ui(self, lang_code: str):
        ...
        self.save_desc.setText(self.i18n_manager.translate("This operation will back up src/ and tests/ folders."))
        self.save_name_input.setPlaceholderText(self.i18n_manager.translate("Backup name (e.g. trading_bot_v1)"))
        self.save_btn.setText(self.i18n_manager.translate("📦 Backup current workspace"))
