    ...
    def handle_generate(self):
        ...
        if not name:
            NotificationDialog.show_message(
                self, "Warning", "Feature Name cannot be empty!"
            )
            return
        ...
        if not platforms:
            NotificationDialog.show_message(
                self, "Warning", "Select at least one platform!"
            )
            return
        if not db_techs:
            NotificationDialog.show_message(
                self, "Warning", "Select at least one DB implementation!"
            )
            return
        ...
        self.main_win.log_info(
            f"Generating feature '{name}' for project [{project_name}]..."
        )
        ...
        try:
            self.app_ctx.generate_feature_controller.execute(
                args, self.root_dir, project_name
            )
