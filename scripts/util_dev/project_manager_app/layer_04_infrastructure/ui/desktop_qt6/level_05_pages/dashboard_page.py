...
from PyQt6.QtWidgets import (
    QWidget, QVBoxLayout, QHBoxLayout, QFrame, QLabel, QListWidget, QListWidgetItem, QStackedWidget
)
...
class DashboardPage(BasePageTemplate):
    ...
    def _init_no_project_view(self):
        ...
        self.lbl_desc = BodyLabel("Workspace is currently empty. Please create a new project or restore a saved session below.")
    ...
    def handle_create_project(self):
        ...
        self.main_win.log_info(f"Scaffolding directory structure and UI skeleton for project [{name}]...")
        ...
        if output.status == "ok":
            if write_project_name(self.root_dir, name):
                ...
                self.main_win.log_success(f"Project [{name}] created and activated successfully!")
    ...
    def refresh_dashboard(self):
        ...
        usecases_dir = os.path.join(self.root_dir, "src", "layer_02_usecases", "usecases")
        features = []
        if os.path.exists(usecases_dir):
            for item in os.listdir(usecases_dir):
                if item != "__pycache__" and os.path.isdir(os.path.join(usecases_dir, item)):
                    features.append(item)
                    
        self.features_list.clear()
        if not features:
            item = QListWidgetItem(self.i18n_manager.translate("No features have been created yet."))
            item.setFlags(item.flags() & ~Qt.ItemFlag.ItemIsSelectable)
            item.setForeground(Qt.GlobalColor.gray)
            self.features_list.addItem(item)
            self.project_stats_label.setText(self.i18n_manager.translate("Project is currently empty."))
        else:
            for feat in sorted(features):
                self.features_list.addItem(f"✨ {feat}")
            self.project_stats_label.setText(
                self.i18n_manager.translate("Project has {count} feature(s) scaffolded.", count=len(features))
            )

    def retranslate_ui(self, lang_code: str):
        self.lbl_desc.setText(self.i18n_manager.translate("Workspace is currently empty. Please create a new project or restore a saved session below."))
        self.lbl_new_title.setText(self.i18n_manager.translate("✨ Create New Project"))
        self.new_proj_input.setPlaceholderText(self.i18n_manager.translate("Project name (e.g. trading_bot)"))
        ...
        self.btn_check_arch.setText(self.i18n_manager.translate("🛡️ Check Architecture"))
        self.btn_backups.setText(self.i18n_manager.translate("💾 View Backups"))
        self.lbl_feats.setText(self.i18n_manager.translate("Generated Features (Use Cases)"))
        self.refresh_dashboard()
