...
class Sidebar(QFrame):
    ...
    def handle_theme_change(self, index):
        ...
        win = self.window()
        if win is not None and hasattr(win, "app_ctx"):
            app_ctx = getattr(win, "app_ctx")
            app_ctx.theme_manager.switch_theme(selected_theme)
    ...
    def handle_mode_change(self, index):
        ...
        if win is not None and hasattr(win, "app_ctx"):
            app_ctx = getattr(win, "app_ctx")
            app_ctx.mode_manager.switch_mode(mode)
