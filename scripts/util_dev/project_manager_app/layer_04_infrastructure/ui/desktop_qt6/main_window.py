...
from scripts.util_dev.project_manager_app.layer_05_bootstrap.app_context_desktop import (
    AppContextDesktop,
)
...
from .level_05_pages.rename_module_page import RenameModulePage
from .level_05_pages.developer_utilities_page import DeveloperUtilitiesPage
...
class MainWindow(QMainWindow):
    def __init__(self, root_dir):
        ...
        self.sidebar_widget = Sidebar(
            root_dir, self.app_ctx.i18n_manager, self.switch_view
        )
        ...
        self.rename_module_page = RenameModulePage(self, self.app_ctx, self.root_dir)
        self.saved_projects_page = SavedProjectsPage(self, self.app_ctx, self.root_dir)
        self.developer_utilities_page = DeveloperUtilitiesPage(
            self, self.app_ctx, self.root_dir
        )
        ...
        self.views_stack.addWidget(self.rename_module_page)  # index 2
        self.views_stack.addWidget(self.saved_projects_page)  # index 3
        self.views_stack.addWidget(self.developer_utilities_page)  # index 4
        self.views_stack.addWidget(self.reset_workspace_page)  # index 5

        main_content_layout.addWidget(self.views_stack, stretch=3)
        ...
        self._update_title()
        self.log_info(
            "System initialized. Welcome to Clean Architecture Project Manager."
        )
    ...
    def _update_title(self):
        if self._project_name:
            self.setWindowTitle(
                f"Clean Architecture Project Manager  —  📦 {self._project_name}"
            )
        else:
    ...
    def log_success(self, text):
        self.console.append_log(
            f"<font color='{SUCCESS_COLOR}'>[SUCCESS] {text}</font>"
        )
