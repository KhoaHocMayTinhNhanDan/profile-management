...
class GenerateFeatureInteractor:
        ...
        try:
            ...
            if not input_data.feature_name.strip():
                self._generate_ui_pages(
                    root, "", "", input_data.platforms, input_data.project_name
                )
                self._update_app_contexts(
                    root, "", "", input_data.platforms, input_data.db_techs
                )
                return GenerateFeatureOutput(
                    "ok", "Project structured scaffolded successfully."
                )
            ...
            self._generate_ui_pages(
                root, pascal, snake, input_data.platforms, input_data.project_name
            )
            ...
            self._update_app_contexts(
                root, pascal, snake, input_data.platforms, input_data.db_techs
            )
            return GenerateFeatureOutput(
    ...
    def _generate_layer_03(self, root, pascal, snake, platforms):
        for plat in platforms:
            ...
            self._write(
                ...
                tp.get_controller_template(pascal, snake, plat),
            )
            ...
            self._write(
                ...
                tp.get_presenter_template(pascal, snake),
            )
    ...
    def _generate_ui_pages(
        self, root, pascal, snake, platforms, project_name: str = ""
    ):
        if "desktop" in platforms:
            ...
            if not self._file_repo.file_exists(main_win_path):
                ...
                page_tpl_path = os.path.join(
                    qt6_dir, "level_04_templates", "page_template.py"
                )
                ...
                self._write(
                    os.path.join(qt6_dir, "level_01_atoms", "buttons.py"),
                    tp.get_ui_pyqt6_atom_buttons_template(),
                )
                self._write(
                    os.path.join(qt6_dir, "level_01_atoms", "inputs.py"),
                    tp.get_ui_pyqt6_atom_inputs_template(),
                )
                self._write(
                    os.path.join(qt6_dir, "level_01_atoms", "labels.py"),
                    tp.get_ui_pyqt6_atom_labels_template(),
                )
                self._write(
                    os.path.join(qt6_dir, "level_02_molecules", "ui_inspector.py"),
        ...
        if "mobile" in platforms:
            kv_dir = os.path.join(
                root, paths.SRC, paths.LAYER_04, paths.UI, "mobile", "kivy"
            )
        ...
        if "web" in platforms or not platforms:
            web_dir = os.path.join(
                root, paths.SRC, paths.LAYER_04, paths.UI, "web", "frontend"
            )
    ...
    def _generate_fastapi_router(self, root, pascal, snake):
        r_dir = os.path.join(
            root, paths.SRC, paths.LAYER_04, paths.UI, "web", "fastapi", "routers"
        )
        self._write(os.path.join(r_dir, "__init__.py"), "")
        self._write(
            os.path.join(r_dir, f"{snake}.py"),
            tp.get_fastapi_router_template(pascal, snake),
        )

        main_file = os.path.join(
            root, paths.SRC, paths.LAYER_04, paths.UI, "web", "fastapi", "main.py"
        )
    ...
    def _generate_tests(self, root, pascal, snake):
        ...
        self._write(
            ...
            tp.get_test_template(pascal, snake),
        )
        ...
        self._write(
            ...
            tp.get_integration_test_template(pascal, snake),
        )
    ...
    def _update_app_contexts(self, root, pascal, snake, platforms, db_techs):
        ...
        for plat in base_plats:
            ...
            orig_plats = [
                p
                for p in platforms
                if p == plat or (plat == "desktop" and "desktop" in p)
            ]
            ...
            bindings += f"        self.container.register(I{pascal}Repository, repo_{camel})\n"
            bindings += f"        interactor_{camel} = {pascal}Interactor(repo_{camel})\n"
            bindings += f"        self.container.register({pascal}Interactor, interactor_{camel})\n"
            bindings += f"        controller_{camel} = {pascal}Controller(interactor_{camel})\n"
