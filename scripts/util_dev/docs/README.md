...
Chạy lệnh CLI của Project Manager App để tự động thiết lập môi trường ảo `.venv` và cài đặt các thư viện cần thiết:
```bash
python scripts/util_dev/project_manager_app/run_project_manager_app/cli/run_cli.py --setup-env
```
*(Lệnh này sẽ tự động khởi tạo môi trường ảo, kích hoạt pip, cài đặt toàn bộ các dependencies phục vụ phát triển, kiểm thử và phân tích kiểu).*
