...
### 5. Rename or Move Modules (Auto-Fix Imports)
To rename/move a Python file or package and automatically fix all imports across the workspace:
```bash
python scripts/util_dev/rename_module.py <target_path> <new_name>
```
*Example (Renaming a file):*
```bash
python scripts/util_dev/rename_module.py src/layer_01_entities/old_entity.py new_entity
```
